...
onMounted(async () => {
    try {
        ...
        const response2 = await axios.get('/data/hcnStateDataAvg.json');
        const stateData = response2.data;
        let {segmentList, statesList} = stateData;
        const hcn = data.hcn;
        const time = data.time.map(d => d[0]);

        console.log(segmentList, statesList)
        ...
        const container = overview2.append('g').attr('class', 'container').attr('transform', 'translate(90, 0)');
        let rectHP = 10;
        container.append('g').attr('class', 'textG')
            .selectAll('text')
            .data(hcn)
            .join('text')
            .attr('x', -120)
            .attr('y', (d, i) => (rectH+rectHP) * (i - 0.5))
            .text((d, i) => `${4043+i}/hcn_ne001`)
        container.append('g').attr('class', 'rectG')
            ...
            .on('click', function (e, data) {
                ...
                // peak point
                lineChartStateG.selectAll('.statePeaksG').remove();
                lineChartStateG.selectAll('.statePeaksG')
                    .data(statesList[idx].map(t => t.peaks))
                    .join('g')
                    .attr('class', 'statePeaksG')
                    .selectAll('.circle')
                    .data(d => d)
                    .join('circle')
                    .attr('class', 'circle')
                ...
                lineChartStateG.selectAll('.stateTrendG')
                    .data(statesList[idx].map(d => d.trend[0]))
                    .join('g')
                    ...
                    .attr('transform', function(d, i) {
                        let rotate = d ? 45 : -45;
                        return `translate(${x(time[segmentList[idx][i]])}, 10) rotate(${rotate})`
                    })
                    .attr('stroke', '#000')
        ...
        //     .data(d => d.map(t => t.trend[0]))
        //     .join('g')
        //     .attr('class', 'stateTrendG')
        //     .append('circle')
        //     .attr('cx', function (d, i) {
        //         let idx = +d3.select(this.parentNode.parentNode).attr('idx');
        //         return x((time[segmentList[idx][i]] + time[segmentList[idx][i+1]]) / 2);
        //     })
        //     .attr('cy', (d, i) => d? 10 : 30)
        //     .attr('r', 2)
        //     .attr('stroke', '#000')
        // stateG.selectAll('.statePeaksG')
        //     .data(d => d.map(t => t.peaks))
        //     .join('g')
        //     .attr('class', 'statePeaksG')
        //     .selectAll('circle')
        //     .data(d => d)
        ...
        // 设置 x 和 y 的比例尺
        const xTrendLine = d3.scaleLinear()
            .domain([xMin, xMax])
            .range([0, rectW]); // 加入一些边距

        const yTrendLine = d3.scaleLinear()
            .domain([yMin, yMax])
            .range([rectH, 0]); // 加入一些边距

        stateG.selectAll('.stateTrendLineG')
            .data(d => d.map(t => [t.trend[1], t.trend[2]]))
            .join('g')
            .attr('class', 'stateTrendLineG')
            .append('line')
            .attr('x1', function (d, i) {
                let idx = +d3.select(this.parentNode.parentNode).attr('idx');
                return x(time[segmentList[idx][i]]);
            })
            .attr('x2', function (d, i) {
                let idx = +d3.select(this.parentNode.parentNode).attr('idx');
                return x(time[segmentList[idx][i+1]]);
            })
            .attr('y1', function (d, i) {
                let idx = +d3.select(this.parentNode.parentNode).attr('idx');
                return yTrendLine(time[segmentList[idx][i]]*d[0]+d[1]);
            })
            .attr('y2', function (d, i) {
                let idx = +d3.select(this.parentNode.parentNode).attr('idx');
                return yTrendLine(time[segmentList[idx][i+1]]*d[0]+d[1]);
            })
            .attr('stroke', '#000');
        ...
        const lineChartStateG = lineChart
            ...
            .attr('class', 'stateG');
        // lineChartStateG.selectAll('.stateTrendG')
        //     .data(statesList[0].map(d => d.trend[0]))
        //     .join('g')
        //     .attr('class', 'stateTrendG')
        //     .append('path')
        //     .attr('d', (d, i) => d ? upArrowPath : downArrowPath)
        ...
        lineChartStateG.selectAll('.statePeaksG')
            .data(statesList[0].map(t => t.peaks))
            .join('g')
            .attr('class', 'statePeaksG')
            .selectAll('.circle')
            .data(d => d)
        ...
        lineChartStateG.append('circle')
            .attr('cx', 0)
            .attr('cy', -40)
            .attr('r', 5)
            .attr('fill', 'red')
            .attr('stroke', 'none');
        lineChartStateG.append('text')
            .attr('x', 20)
            .attr('y', -37)
            .text('peak')
